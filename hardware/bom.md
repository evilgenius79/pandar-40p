...
| 7 | XIAO ESP32-S3 | **in service — IMU only since 2026-08-06** | v2 firmware, ~201 Hz. D5 (GPS RX) and D4 (PPS) are now unused: the M10 was removed and GNSS moved to USB. Firmware still emits `0xAA 0x56`/`0x57` packet types if anything ever drives those pins; measured 2026-08-06 it emits neither. `PIN_PPS` stays `INPUT_PULLDOWN` — that fix killed the "PPS flood" (a floating pin, never a GPS quirk) and matters more now that nothing drives D4 |
...
| 10 | u-blox M10 GNSS | **REMOVED from the rig 2026-08-06** | L1-only, metre-class, fed `/gps/fix` via the XIAO's D5. Superseded by #15 and physically removed — it could never use the InCORS streams, which are L1+L2 (`carrier=2`). D5 on the XIAO is now unused |
